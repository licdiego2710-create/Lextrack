...
export default function Plazos() {
  ...
  // ── Generar link WhatsApp para un expediente ──────────────────────
  function whatsappTermino(e) {
    const fechaRef = e.termino || e.proxima_fecha
    const d = diasHasta(fechaRef)
    const cuandoLabel = d === null ? '' : d < 0 ? `(VENCIDO hace ${Math.abs(d)} días)` : d === 0 ? '⚠️ VENCE HOY' : `vence en ${d} día(s)`
    const msg = [
      `*LexTrack MX — Aviso de Término*`,
      ``,
      `📁 Expediente: *${e.num}*`,
      `⚖️ ${e.actor} vs. ${e.demandado}`,
      e.etapa ? `📌 Etapa: ${e.etapa}` : '',
      e.juzgado ? `🏛️ ${e.juzgado}` : '',
      ``,
      `📅 Fecha de término: *${fmtFecha(fechaRef)}* ${cuandoLabel}`,
      e.abogado_responsable ? `👤 Responsable: ${e.abogado_responsable}` : '',
    ].filter(Boolean).join('\n')
              ...
              {expsFiltrados.map((e, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* WhatsApp */}
                    <a
                      href={whatsappTermino(e)}
                      target="_blank"
                      rel="noopener noreferrer"
                      onClick={ev => ev.stopPropagation()}
                      style={{ color: '#25D366', flexShrink: 0, display: 'flex', alignItems: 'center' }}
